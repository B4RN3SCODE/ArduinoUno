 ...
 * SerialPort
 *
 * Tests getting data back from shit
 *
...
void loop() {
	...
	if(Serial.available() > 0) {       //Receive serial data
		...
		int tmpVal = (int)receiveVal;
		if(tmpVal % 2 == 0) {          //Receive data is 1, lit LED lights
			digitalWrite(ledpin,HIGH); //print out the value of the LED
			Serial.println("LED:ON"); //send data to the serial monitor
		}
		if(tmpVal % 2 == 1) {          //Receive data is 0, off LED lights
